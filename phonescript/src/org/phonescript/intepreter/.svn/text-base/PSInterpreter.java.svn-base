...
public class PSInterpreter  {	
	...
	NewStack arrays = new NewStack();	
	NewStack loops = new NewStack();	
	...
	public void exec(String string) {
		CharBuffer buffer = new CharBuffer(string ,2);
		exec(buffer);
		
	}
	public void exec(CharBuffer buffer) {	
		while(true) {
			PSToken token = PSToken.getToken(this, buffer);
			if (stackMode)
...
//				break;
	...
	final public Hashtable names = new Hashtable();
	
	public void execute(CharBuffer buffer) {
