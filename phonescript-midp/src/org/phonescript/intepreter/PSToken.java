...
//	}	
	...
	static PSToken getToken(CharBuffer buffer) {
		...
		while (token == null) {
			...
			if (ch == '/')
				if (buffer.peek(2) == '/')
					return processName(buffer, (char) ch, IMEDIATENAME);
				else
					return processName(buffer, (char) ch, LITERALNAME);
			
			if ( (ch =='-' || Character.isDigit((char)ch)) && (token = processNUMBER(buffer, (char) ch)) != null)
				return token;
			
			if ( (token = processName(buffer, (char) ch, NAME)) != null)
				return token;
//
//			
//			
...
//				return processCHAR(buffer, (char) ch);
	...
	private static PSToken processName(CharBuffer buffer, char ch, int type) {
		StringBuffer nameStr = new StringBuffer();
		do {
			nameStr.append(ch);
			buffer.consume();
			ch = (char) buffer.peek(1);
		} while (false == Character.isWhitespace(ch) && ch != '%');
